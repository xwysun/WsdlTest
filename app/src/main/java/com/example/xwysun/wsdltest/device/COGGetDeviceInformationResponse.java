package com.example.xwysun.wsdltest.device;

//----------------------------------------------------
//
// Generated by www.easywsdl.com
// Version: 5.2.8.3
//
// Created by Quasar Development 
//
//---------------------------------------------------


import java.util.Hashtable;
import org.ksoap2.serialization.*;

public class COGGetDeviceInformationResponse extends AttributeContainer implements KvmSerializable
{

    
    public String Manufacturer;
    
    public String Model;
    
    public String FirmwareVersion;

    public String SerialNumber;

    public String HardwareId;
    private transient java.lang.Object __source;    
    

    
    
    
    public void loadFromSoap(java.lang.Object paramObj,COGExtendedSoapSerializationEnvelope __envelope)
    {
        if (paramObj == null)
            return;
        AttributeContainer inObj=(AttributeContainer)paramObj;
        __source=inObj; 
        if(inObj instanceof SoapObject)
        {
            SoapObject soapObject=(SoapObject)inObj;
            int size = soapObject.getPropertyCount();
            for (int i0=0;i0< size;i0++)
            {
                PropertyInfo info=soapObject.getPropertyInfo(i0);
                if(!loadProperty(info,soapObject,__envelope))
                {
                }
            } 
        }

    }

    
    protected boolean loadProperty(PropertyInfo info,SoapObject soapObject,COGExtendedSoapSerializationEnvelope __envelope)
    {
        java.lang.Object obj = info.getValue();
        if (info.name.equals("Manufacturer"))
        {
            if(obj!=null)
            {
                if (obj.getClass().equals(SoapPrimitive.class))
                {
                    SoapPrimitive j =(SoapPrimitive) obj;
                    if(j.toString()!=null)
                    {
                        this.Manufacturer = j.toString();
                    }
                }
                else if (obj instanceof String){
                    this.Manufacturer = (String)obj;
                }
            }
            return true;
        }
        if (info.name.equals("Model"))
        {
            if(obj!=null)
            {
                if (obj.getClass().equals(SoapPrimitive.class))
                {
                    SoapPrimitive j =(SoapPrimitive) obj;
                    if(j.toString()!=null)
                    {
                        this.Model = j.toString();
                    }
                }
                else if (obj instanceof String){
                    this.Model = (String)obj;
                }
            }
            return true;
        }
        if (info.name.equals("FirmwareVersion"))
        {
            if(obj!=null)
            {
                if (obj.getClass().equals(SoapPrimitive.class))
                {
                    SoapPrimitive j =(SoapPrimitive) obj;
                    if(j.toString()!=null)
                    {
                        this.FirmwareVersion = j.toString();
                    }
                }
                else if (obj instanceof String){
                    this.FirmwareVersion = (String)obj;
                }
            }
            return true;
        }
        if (info.name.equals("SerialNumber"))
        {
            if(obj!=null)
            {
                if (obj.getClass().equals(SoapPrimitive.class))
                {
                    SoapPrimitive j =(SoapPrimitive) obj;
                    if(j.toString()!=null)
                    {
                        this.SerialNumber = j.toString();
                    }
                }
                else if (obj instanceof String){
                    this.SerialNumber = (String)obj;
                }
            }
            return true;
        }
        if (info.name.equals("HardwareId"))
        {
            if(obj!=null)
            {
                if (obj.getClass().equals(SoapPrimitive.class))
                {
                    SoapPrimitive j =(SoapPrimitive) obj;
                    if(j.toString()!=null)
                    {
                        this.HardwareId = j.toString();
                    }
                }
                else if (obj instanceof String){
                    this.HardwareId = (String)obj;
                }
            }
            return true;
        }
        return false;
    }
    
    public java.lang.Object getOriginalXmlSource()
    {
        return __source;
    }    
    

    @Override
    public java.lang.Object getProperty(int propertyIndex) {
        //!!!!! If you have a compilation error here then you are using old version of ksoap2 library. Please upgrade to the latest version.
        //!!!!! You can find a correct version in Lib folder from generated zip file!!!!!
        if(propertyIndex==0)
        {
            return Manufacturer;
        }
        if(propertyIndex==1)
        {
            return Model;
        }
        if(propertyIndex==2)
        {
            return FirmwareVersion;
        }
        if(propertyIndex==3)
        {
            return SerialNumber;
        }
        if(propertyIndex==4)
        {
            return HardwareId;
        }
        return null;
    }


    @Override
    public int getPropertyCount() {
        return 5;
    }

    @Override
    public void getPropertyInfo(int propertyIndex, @SuppressWarnings("rawtypes") Hashtable arg1, PropertyInfo info)
    {
        if(propertyIndex==0)
        {
            info.type = PropertyInfo.STRING_CLASS;
            info.name = "Manufacturer";
            info.namespace= "http://www.onvif.org/ver10/device/wsdl";
        }
        if(propertyIndex==1)
        {
            info.type = PropertyInfo.STRING_CLASS;
            info.name = "Model";
            info.namespace= "http://www.onvif.org/ver10/device/wsdl";
        }
        if(propertyIndex==2)
        {
            info.type = PropertyInfo.STRING_CLASS;
            info.name = "FirmwareVersion";
            info.namespace= "http://www.onvif.org/ver10/device/wsdl";
        }
        if(propertyIndex==3)
        {
            info.type = PropertyInfo.STRING_CLASS;
            info.name = "SerialNumber";
            info.namespace= "http://www.onvif.org/ver10/device/wsdl";
        }
        if(propertyIndex==4)
        {
            info.type = PropertyInfo.STRING_CLASS;
            info.name = "HardwareId";
            info.namespace= "http://www.onvif.org/ver10/device/wsdl";
        }
    }
    
    @Override
    public void setProperty(int arg0, java.lang.Object arg1)
    {
    }

    
}

