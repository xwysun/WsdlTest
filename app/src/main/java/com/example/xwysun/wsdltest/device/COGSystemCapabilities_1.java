package com.example.xwysun.wsdltest.device;

//----------------------------------------------------
//
// Generated by www.easywsdl.com
// Version: 5.2.8.3
//
// Created by Quasar Development 
//
//---------------------------------------------------


import java.util.Hashtable;
import org.ksoap2.serialization.*;

public class COGSystemCapabilities_1 extends AttributeContainer implements KvmSerializable
{

    
    public Boolean DiscoveryResolve=false;
    
    public Boolean DiscoveryBye=false;
    
    public Boolean RemoteDiscovery=false;
    
    public Boolean SystemBackup=false;
    
    public Boolean SystemLogging=false;
    
    public Boolean FirmwareUpgrade=false;
    
    public Boolean HttpFirmwareUpgrade=false;
    
    public Boolean HttpSystemBackup=false;
    
    public Boolean HttpSystemLogging=false;
    
    public Boolean HttpSupportInformation=false;
    
    public Boolean StorageConfiguration=false;
    
    public Integer MaxStorageConfigurations=0;
    
    public Integer GeoLocationEntries=0;
    
    public String AutoGeo;
    
    public String StorageTypesSupported;
    private transient java.lang.Object __source;    
    

    
    
    
    public void loadFromSoap(java.lang.Object paramObj,COGExtendedSoapSerializationEnvelope __envelope)
    {
        if (paramObj == null)
            return;
        AttributeContainer inObj=(AttributeContainer)paramObj;
        __source=inObj; 
        if(inObj instanceof SoapObject)
        {
            SoapObject soapObject=(SoapObject)inObj;
            int size = soapObject.getPropertyCount();
            for (int i0=0;i0< size;i0++)
            {
                PropertyInfo info=soapObject.getPropertyInfo(i0);
                if(!loadProperty(info,soapObject,__envelope))
                {
                }
            } 
        }
        if (inObj.hasAttribute("DiscoveryResolve"))
        {	
            java.lang.Object j = inObj.getAttribute("DiscoveryResolve");
            if (j != null)
            {
                DiscoveryResolve = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("DiscoveryBye"))
        {	
            java.lang.Object j = inObj.getAttribute("DiscoveryBye");
            if (j != null)
            {
                DiscoveryBye = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("RemoteDiscovery"))
        {	
            java.lang.Object j = inObj.getAttribute("RemoteDiscovery");
            if (j != null)
            {
                RemoteDiscovery = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("SystemBackup"))
        {	
            java.lang.Object j = inObj.getAttribute("SystemBackup");
            if (j != null)
            {
                SystemBackup = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("SystemLogging"))
        {	
            java.lang.Object j = inObj.getAttribute("SystemLogging");
            if (j != null)
            {
                SystemLogging = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("FirmwareUpgrade"))
        {	
            java.lang.Object j = inObj.getAttribute("FirmwareUpgrade");
            if (j != null)
            {
                FirmwareUpgrade = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("HttpFirmwareUpgrade"))
        {	
            java.lang.Object j = inObj.getAttribute("HttpFirmwareUpgrade");
            if (j != null)
            {
                HttpFirmwareUpgrade = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("HttpSystemBackup"))
        {	
            java.lang.Object j = inObj.getAttribute("HttpSystemBackup");
            if (j != null)
            {
                HttpSystemBackup = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("HttpSystemLogging"))
        {	
            java.lang.Object j = inObj.getAttribute("HttpSystemLogging");
            if (j != null)
            {
                HttpSystemLogging = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("HttpSupportInformation"))
        {	
            java.lang.Object j = inObj.getAttribute("HttpSupportInformation");
            if (j != null)
            {
                HttpSupportInformation = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("StorageConfiguration"))
        {	
            java.lang.Object j = inObj.getAttribute("StorageConfiguration");
            if (j != null)
            {
                StorageConfiguration = new Boolean(j.toString());
            }
        }
        if (inObj.hasAttribute("MaxStorageConfigurations"))
        {	
            java.lang.Object j = inObj.getAttribute("MaxStorageConfigurations");
            if (j != null)
            {
                MaxStorageConfigurations = Integer.parseInt(j.toString());
            }
        }
        if (inObj.hasAttribute("GeoLocationEntries"))
        {	
            java.lang.Object j = inObj.getAttribute("GeoLocationEntries");
            if (j != null)
            {
                GeoLocationEntries = Integer.parseInt(j.toString());
            }
        }
        if (inObj.hasAttribute("AutoGeo"))
        {	
            java.lang.Object j = inObj.getAttribute("AutoGeo");
            if (j != null)
            {
                AutoGeo = j.toString();
            }
        }
        if (inObj.hasAttribute("StorageTypesSupported"))
        {	
            java.lang.Object j = inObj.getAttribute("StorageTypesSupported");
            if (j != null)
            {
                StorageTypesSupported = j.toString();
            }
        }

    }

    
    protected boolean loadProperty(PropertyInfo info,SoapObject soapObject,COGExtendedSoapSerializationEnvelope __envelope)
    {
        java.lang.Object obj = info.getValue();
        return false;
    }
    
    public java.lang.Object getOriginalXmlSource()
    {
        return __source;
    }    
    

    @Override
    public java.lang.Object getProperty(int propertyIndex) {
        //!!!!! If you have a compilation error here then you are using old version of ksoap2 library. Please upgrade to the latest version.
        //!!!!! You can find a correct version in Lib folder from generated zip file!!!!!
        return null;
    }


    @Override
    public int getPropertyCount() {
        return 0;
    }

    @Override
    public void getPropertyInfo(int propertyIndex, @SuppressWarnings("rawtypes") Hashtable arg1, PropertyInfo info)
    {
    }
    
    @Override
    public void setProperty(int arg0, java.lang.Object arg1)
    {
    }



    @Override
    public int getAttributeCount() {
        return 15;
    }
    
    @Override
    public void getAttributeInfo(int index, AttributeInfo info) {
        if(index==0)
        {
            info.name = "DiscoveryResolve";
            info.namespace= "";
            if(this.DiscoveryResolve!=null)
            {
                info.setValue(this.DiscoveryResolve);
            }
            
        }
        if(index==1)
        {
            info.name = "DiscoveryBye";
            info.namespace= "";
            if(this.DiscoveryBye!=null)
            {
                info.setValue(this.DiscoveryBye);
            }
            
        }
        if(index==2)
        {
            info.name = "RemoteDiscovery";
            info.namespace= "";
            if(this.RemoteDiscovery!=null)
            {
                info.setValue(this.RemoteDiscovery);
            }
            
        }
        if(index==3)
        {
            info.name = "SystemBackup";
            info.namespace= "";
            if(this.SystemBackup!=null)
            {
                info.setValue(this.SystemBackup);
            }
            
        }
        if(index==4)
        {
            info.name = "SystemLogging";
            info.namespace= "";
            if(this.SystemLogging!=null)
            {
                info.setValue(this.SystemLogging);
            }
            
        }
        if(index==5)
        {
            info.name = "FirmwareUpgrade";
            info.namespace= "";
            if(this.FirmwareUpgrade!=null)
            {
                info.setValue(this.FirmwareUpgrade);
            }
            
        }
        if(index==6)
        {
            info.name = "HttpFirmwareUpgrade";
            info.namespace= "";
            if(this.HttpFirmwareUpgrade!=null)
            {
                info.setValue(this.HttpFirmwareUpgrade);
            }
            
        }
        if(index==7)
        {
            info.name = "HttpSystemBackup";
            info.namespace= "";
            if(this.HttpSystemBackup!=null)
            {
                info.setValue(this.HttpSystemBackup);
            }
            
        }
        if(index==8)
        {
            info.name = "HttpSystemLogging";
            info.namespace= "";
            if(this.HttpSystemLogging!=null)
            {
                info.setValue(this.HttpSystemLogging);
            }
            
        }
        if(index==9)
        {
            info.name = "HttpSupportInformation";
            info.namespace= "";
            if(this.HttpSupportInformation!=null)
            {
                info.setValue(this.HttpSupportInformation);
            }
            
        }
        if(index==10)
        {
            info.name = "StorageConfiguration";
            info.namespace= "";
            if(this.StorageConfiguration!=null)
            {
                info.setValue(this.StorageConfiguration);
            }
            
        }
        if(index==11)
        {
            info.name = "MaxStorageConfigurations";
            info.namespace= "";
            if(this.MaxStorageConfigurations!=null)
            {
                info.setValue(this.MaxStorageConfigurations);
            }
            
        }
        if(index==12)
        {
            info.name = "GeoLocationEntries";
            info.namespace= "";
            if(this.GeoLocationEntries!=null)
            {
                info.setValue(this.GeoLocationEntries);
            }
            
        }
        if(index==13)
        {
            info.name = "AutoGeo";
            info.namespace= "";
            if(this.AutoGeo!=null)
            {
                info.setValue(this.AutoGeo);
            }
            
        }
        if(index==14)
        {
            info.name = "StorageTypesSupported";
            info.namespace= "";
            if(this.StorageTypesSupported!=null)
            {
                info.setValue(this.StorageTypesSupported);
            }
            
        }
    }

    @Override
    public void getAttribute(int i, AttributeInfo attributeInfo) {

    }

    @Override
    public void setAttribute(AttributeInfo attributeInfo) {

    }    
}

